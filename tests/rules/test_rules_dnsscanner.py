...
class DnsScanTest(RulesBase):
    ...
    def test_dnsscan_one_target(self):
        self.engine.declare(DomainTcpIpService(addr='10.1.1.1', port=53, secure=False))
        self.engine.declare(DomainUdpIpService(addr='10.1.1.1', port=53, secure=False))
        self.engine.run()
        assertFactNotIn(ScanNeeded(category=ToolCategory.dns_scanner, addr=ScanNeeded.ANY), self.engine)
        assertFactIn(ScanNeeded(category=ToolCategory.dns_scanner, addr='10.1.1.1', port=53, secure=False),
                     self.engine)

    def test_dnsscan_two_target(self):
        self.engine.declare(DomainUdpIpService(addr='10.1.1.1', port=53, secure=False))
        self.engine.declare(DomainUdpIpService(addr='10.1.1.2', port=53, secure=False))
        self.engine.run()
        assertFactNotIn(ScanNeeded(category=ToolCategory.dns_scanner, addr=ScanNeeded.ANY), self.engine)
        assertFactIn(ScanNeeded(category=ToolCategory.dns_scanner, addr='10.1.1.1', port=53, secure=False),
                     self.engine)
        assertFactIn(ScanNeeded(category=ToolCategory.dns_scanner, addr='10.1.1.2', port=53, secure=False),
                     self.engine)
