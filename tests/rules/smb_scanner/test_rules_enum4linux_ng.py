...
class Enum4LinuxNgRulesTest(RulesBase):
    ...
    def test_enum4linux_ng(self):
        ...
        self.engine.declare(ScanPresent(category=ToolCategory.smb_scanner, addr='10.129.229.189',
                                        name=Enum4LinuxNgRules.enum4linux_ng_tool_name))
        self.engine.run()
        assertFactNotIn(ScanNeeded(category=ToolCategory.smb_scanner, addr='10.129.229.189'), self.engine)
        assertFactNotIn(
            ToolRecommended(category=ToolCategory.smb_scanner, name=Enum4LinuxNgRules.enum4linux_ng_tool_name),
            self.engine)
