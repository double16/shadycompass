...
class EtcHostsReaderTest(unittest.TestCase):
    ...
    def test_read_hosts(self):
        ...
        self.assertIn(HostnameIPv4Resolution(hostname='localhost', addr='127.0.0.1'), facts)
        self.assertIn(HostnameIPv6Resolution(hostname='localhost', addr='::1'), facts)
        self.assertIn(HostnameIPv4Resolution(hostname='target1.local', addr='10.10.1.1'), facts)
        self.assertIn(HostnameIPv4Resolution(hostname='target2.local', addr='10.10.1.2'), facts)
        self.assertIn(HostnameIPv4Resolution(hostname='subdomain.target2.local', addr='10.10.1.2'), facts)
